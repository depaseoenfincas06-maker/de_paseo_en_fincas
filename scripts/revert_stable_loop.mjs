...
function patchCodeNode(node) {
  ...
  if (!jsCode.includes("const hasCustomerFacingMessage = Boolean(String(finalWhatsappText || '').trim());")) {
    jsCode = jsCode.replace(anchor, insert);
  }
...
patchCodeNode(findNode(workflow, 'Code in JavaScript1'));
...
replaceMainConnection(workflow.connections, 'Insert OUTBOUND message (messages)', 'If2');
